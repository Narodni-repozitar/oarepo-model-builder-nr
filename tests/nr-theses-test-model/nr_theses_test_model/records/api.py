...
from nr_theses_test_model.records.dumper import NrThesesTestModelDumper
from nr_theses_test_model.records.models import NrThesesTestModelMetadata
from oarepo_runtime.relations import InternalRelation, PIDRelation, RelationsField


class NrThesesTestModelRecord(Record):
    model_cls = NrThesesTestModelMetadata

    schema = ConstantField(
        "$schema", "http://localhost/schemas/nr_theses_test_model-1.0.0.json"
    )

    index = IndexField("nr_theses_test_model-nr_theses_test_model-1.0.0")
    ...
    dumper_extensions = []
    dumper = NrThesesTestModelDumper(extensions=dumper_extensions)
