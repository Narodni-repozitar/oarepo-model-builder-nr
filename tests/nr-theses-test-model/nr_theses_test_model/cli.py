...
from nr_theses_test_model.proxies import current_service
from nr_theses_test_model.records.api import NrThesesTestModelRecord


@click.group(name="nr-theses-test-model")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = NrThesesTestModelRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(NrThesesTestModelRecord.model_cls.id).filter(
            NrThesesTestModelRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = NrThesesTestModelRecord.get_record(rec_id)
