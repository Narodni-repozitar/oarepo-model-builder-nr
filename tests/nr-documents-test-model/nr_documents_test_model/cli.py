...
from nr_documents_test_model.proxies import current_service
from nr_documents_test_model.records.api import NrDocumentsTestModelRecord


@click.group(name="nr-documents-test-model")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = NrDocumentsTestModelRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(NrDocumentsTestModelRecord.model_cls.id).filter(
            NrDocumentsTestModelRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = NrDocumentsTestModelRecord.get_record(rec_id)
