...
from nr_common_test_model.proxies import current_service
from nr_common_test_model.records.api import NrCommonTestModelRecord


@click.group(name="nr-common-test-model")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = NrCommonTestModelRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(NrCommonTestModelRecord.model_cls.id).filter(
            NrCommonTestModelRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = NrCommonTestModelRecord.get_record(rec_id)
